...
from reid.models.vip_pvt import pvt_v2_b2_p
...
class Baseline(nn.Module):
    ...
    def __init__(self, num_classes, last_stride, model_path, neck, neck_feat, model_name, height, width):
        ...
        if model_name == 'resnet18':
            ...
            self.in_planes = 128
            model_path = osp.join(model_path, "pvt_v2_b2.pth")
            self.base = pvt_v2_b2_p()
        elif model_name == 'rga':
            self.in_planes = 2048
            model_path = osp.join(model_path, "resnet50-19c8e357.pth")
            self.base = ResNet50_RGA_Model(height=height, width=width)
        elif model_name == 'hpm':
            self.in_planes = 2048
            model_path = osp.join(model_path, "resnet50-19c8e357.pth")
            self.base = HPM(num_classes)
        elif model_name == 'rgap':
            self.in_planes = 2048
            model_path = osp.join(model_path, "resnet50-19c8e357.pth")
            ...
            self.in_planes = 512
            model_path = osp.join(model_path, "osnet_x1_0_imagenet.pth")
            self.base = osnet_x1_0(num_classes=num_classes)
